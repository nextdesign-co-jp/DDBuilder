...
public class BdCodeCaseJavaFieldJavaPrimitiveClass extends BdBaseCodeCase {

	/**
	 * コンストラクタ
	 * @param lineListOfCase
	 */
	public BdCodeCaseJavaFieldJavaPrimitiveClass(List<BdCodeLine> lineListOfCase){
	...
	private List<BdStringHelperReplacementPattern> createReplacementPatternList(BdUserAttribute a){
		...
		resultList.add(new BdStringHelperReplacementPattern("Integer", a.getNdAttributeType().getTypeName().getSimpleName()));
		return resultList;
	}
	...
	private List<BdStringHelperReplacementPattern> createReplacementPatternList(BdUserServiceMethodParameter p){
		List<BdStringHelperReplacementPattern> resultList = new ArrayList<BdStringHelperReplacementPattern>();
		resultList.add(new BdStringHelperReplacementPattern("Integer型属性名", p.getTitle()));
		resultList.add(new BdStringHelperReplacementPattern("paramIntegerAttribute", p.getName()));
		resultList.add(new BdStringHelperReplacementPattern("integerAttribute", p.getName()));
		resultList.add(new BdStringHelperReplacementPattern("Integer", p.getMethodParameterType().getTypeName().getSimpleName()));
		return resultList;
	}
	...
	private List<BdStringHelperReplacementPattern> createReplacementPatternList(BdUserServiceMethodResult r){
		...
		resultList.add(new BdStringHelperReplacementPattern("integer", r.getLowerStartedSimpleOrCollectionElementTypeName()));
		return resultList;
	}
}
