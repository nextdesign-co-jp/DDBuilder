...
public class BdCodeCaseHtmlFieldEnum extends BdBaseCodeCase {

	/**
	 * コンストラクタ
	 * @param lineListOfCase
	 */
	public BdCodeCaseHtmlFieldEnum(List<BdCodeLine> lineListOfCase){
	...
	private List<BdStringHelperReplacementPattern> createReplacementPatternList(BdUserAttribute a){
		...
		resultList.add(new BdStringHelperReplacementPattern("Enum型属性名", a.getJavadocTitle()));
		return resultList;
	}
	/** 引数に対応した生成コードを応答する */
	@Override
	...
	private List<BdStringHelperReplacementPattern> createReplacementPatternList(BdUserServiceMethodParameter p){
		...
		resultList.add(new BdStringHelperReplacementPattern("Enum型属性名", p.getTitle()));
		return resultList;
	}
}
