...
public class BdCodeFileSingleSelectPanelHtml extends BdBaseCodeFile {
	 ...
	 */
	public BdCodeFileSingleSelectPanelHtml(File file, BdTemplateProject project) throws BdBuilderException, BdApplicationPropertyException{
		super(file, project);
	}
	 ...
	 * @return
	 */
	private List<String> generateLines(BdUserServiceMethod m, BdUserServiceMethodParameter p, List<BdStringHelperReplacementPattern> createReplacementPatternList) {
		...
		while (lineIndex < this.getCodeLineList().size()){
			...
			if (bdCodeLine.hasCodeBlock()){ //2015.7.3時点ではBLOCKなし
				...
				resultList.add(this.customize(bdCodeLine.getLine(), m, createReplacementPatternList));
				lineIndex++;
			}
		}
		return resultList;
	 ...
	 * 文字列を置き換える
	 */
	private String customize(String line, BdUserServiceMethod m, List<BdStringHelperReplacementPattern> createReplacementPatternList){
		return line;
	}
