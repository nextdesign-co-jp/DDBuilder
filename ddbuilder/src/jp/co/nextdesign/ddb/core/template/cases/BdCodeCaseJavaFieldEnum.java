...
public class BdCodeCaseJavaFieldEnum extends BdBaseCodeCase {
	/**
	 * コンストラクタ
	 * @param lineListOfCase
	 */
	public BdCodeCaseJavaFieldEnum(List<BdCodeLine> lineListOfCase){
	...
	private List<BdStringHelperReplacementPattern> createReplacementPatternList(BdUserAttribute a){
		...
		resultList.add(new BdStringHelperReplacementPattern("attEnum", a.getName()));
		resultList.add(new BdStringHelperReplacementPattern("enumLanguageList", a.getName() + "EnumList"));
		return resultList;
	}
	...
	private List<BdStringHelperReplacementPattern> createReplacementPatternList(BdUserServiceMethodParameter p){
		List<BdStringHelperReplacementPattern> resultList = new ArrayList<BdStringHelperReplacementPattern>();
		resultList.add(new BdStringHelperReplacementPattern("paramAttEnum", p.getName()));
		resultList.add(new BdStringHelperReplacementPattern("言語", p.getTitle()));
		resultList.add(new BdStringHelperReplacementPattern("enumLanguageList", p.getName() + "List"));
		resultList.add(new BdStringHelperReplacementPattern("EnumLanguage", p.getMethodParameterType().getTypeName().getSimpleName()));
		resultList.add(new BdStringHelperReplacementPattern("attEnumChoiceRenderer", p.getName() + "ChoiceRenderer"));
		resultList.add(new BdStringHelperReplacementPattern("attEnumModel", p.getName() + "Model"));
		resultList.add(new BdStringHelperReplacementPattern("attEnum", p.getName()));
		return resultList;
	}
	...
	private List<BdStringHelperReplacementPattern> createReplacementPatternList(BdUserServiceMethodResult r){
		...
		resultList.add(new BdStringHelperReplacementPattern("enum", r.getLowerStartedSimpleOrCollectionElementTypeName()));
		return resultList;
	}
}
