...
public class BdCodeCaseHtmlFieldBoolean extends BdBaseCodeCase {

	/**
	 * コンストラクタ
	 * @param lineListOfCase
	 */
	public BdCodeCaseHtmlFieldBoolean(List<BdCodeLine> lineListOfCase){
	...
	private List<BdStringHelperReplacementPattern> createReplacementPatternList(BdUserAttribute a){
		...
		resultList.add(new BdStringHelperReplacementPattern("キャンペーン1", a.getJavadocTitle()));
		return resultList;
	}
	...
	private List<BdStringHelperReplacementPattern> createReplacementPatternList(BdUserServiceMethodParameter p){
		...
		resultList.add(new BdStringHelperReplacementPattern("キャンペーン1", p.getTitle()));
		return resultList;
	}
}
